...
def test():
    ...
    N, C, H, W = 8, 12, 256, 256

    current_kspace = torch.randn(N, C, H, W, 2)
    sampling_mask = torch.randint(0, 2, (N, 1, H, W, 1))
    sensitivity_map = torch.randn(N, C, H, W, 2)

    out = model(current_kspace,sampling_mask,sensitivity_map)
    print(out.shape)

if __name__ == "__main__":
    # Generate random complex k-space data
    test()
