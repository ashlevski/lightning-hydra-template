...
class MultiVisitNet(nn.Module):
    def __init__(self, single_visit_net, weights_path, multi_visit_net,freeze=True):
        ...
        self.unet = UnetModel2d(in_channels=2,out_channels=1,num_filters=8,num_pool_layers=2,dropout_probability=0)
    ...
    def forward(self, x):
        ...
        # plt.show()+ 0*multi_visit_output
        return output_image + self.unet(torch.stack([output_image,multi_visit_output],dim=1)).squeeze(), output_kspace, target_img
