...
class SliceDataset(Dataset):
    def __init__(self, base_directory, data_df, split, smaps, us_masks, target_type, coils, data_transforms=None,
    ...
    def __getitem__(self, idx):
        ...
        img_path_ = path.join(self.base_directory, str(img_path['split']), 'target_slice_images',
                              str(img_path['patient_id']) + '_s' + str(img_path['slice']) + '_nlinv_img.npy')
        ...
        input_kspace = np.fft.fftshift(np.fft.fft2(np.fft.ifftshift(target_img * smap + noise, axes=(-1, -2))),
                                       axes=(-1, -2)) * mask
        ...
        input_img = np.moveaxis(input_img, 0,
                                -1)  # In numpy, want channels at end. Torch tensor transform will move them to the front
        # target_img = np.moveaxis(target_img, 0, -1)
        # smap = np.moveaxis(smap, 0, -1) # standarize to be
        ...
        if self.target_transforms:
            target_img = torch.from_numpy(target_img)  # self.target_transforms(target_img)
            smap = torch.from_numpy(smap)  # Natalia casted it using tranform which results in (2*N
...
class SliceTestDataset(Dataset):
    def __init__(self, data_df, split, smaps, us_masks, target_type, channels, data_transforms=None,
                 target_transforms=None, rand='smap', smap_choice=None, mask_choice=None) -> None:
        super().__init__()
        '''
