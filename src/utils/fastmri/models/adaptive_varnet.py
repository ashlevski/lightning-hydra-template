...
import fastmri
from src.utils.fastmri.data import transforms
...
class AdaptiveSensitivityModel(nn.Module):
    ...
    def divide_root_sum_of_squares(self, x: torch.Tensor) -> torch.Tensor:
        return x / src.utils.fastmri.rss_complex(x, dim=1).unsqueeze(-1).unsqueeze(1)
...
class AdaptiveVarNet(nn.Module):
    ...
    def forward(
        ...
        current_recon = src.utils.fastmri.complex_abs(
        ...
        output = src.utils.fastmri.rss(fastmri.complex_abs(fastmri.ifft2c(kspace_pred)), dim=1)
    ...
    def sens_reduce(self, x: torch.Tensor, sens_maps: torch.Tensor) -> torch.Tensor:
        x = src.utils.fastmri.ifft2c(x)
        return src.utils.fastmri.complex_mul(x, src.utils.fastmri.complex_conj(sens_maps)).sum(
            dim=1, keepdim=True
        )
...
class AdaptiveVarNetBlock(nn.Module):
    ...
    def sens_expand(self, x: torch.Tensor, sens_maps: torch.Tensor) -> torch.Tensor:
        return src.utils.fastmri.fft2c(fastmri.complex_mul(x, sens_maps))
