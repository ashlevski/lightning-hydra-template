...
class UnetDataTransform:
    ...
    def __call__(
        ...
        image = src.utils.fastmri.ifft2c(masked_kspace)
        ...
        # absolute value
        image = src.utils.fastmri.complex_abs(image)

        # apply Root-Sum-of-Squares if multicoil data
        if self.which_challenge == "multicoil":
            image = src.utils.fastmri.rss(image)
...
class MiniCoilTransform:
    ...
    def __call__(self, kspace, mask, target, attrs, fname, slice_num):
        ...
        kspace = src.utils.fastmri.fft2c(
        ...
        target = src.utils.fastmri.rss_complex(fastmri.ifft2c(to_tensor(kspace)))
